                    ...
                    <div class="statbox widget box box-shadow">
                        <div class="widget-header">                                
                            <div class="row clearfix">
                                <h4 id="popup_title">Create Designation</h4>                                                                      
                            </div>
                        </div>
                                ...
                                <?php if($results){ ?>
                                    <?php foreach($results as $key => $result){ ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if($result->designation_status == 0){ ?>
                                            ...
                                            <td>
                                                <a herf="javascript:void(0);" onclick="edit(<?php echo $result->designation_id;?>);"><i style="font-size:25px" class="fa fa-edit"></i></a>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                <?php } ?>
...
$(document).ready(function(){
    $('#popup_title').html('');
    $('#popup_btn').html('');
    $('#popup_title').html('Create Designation');
    $('#popup_btn').html('SAVE');
    $('#hidden_id').val(0); 
    $('#designation').val('');
    $('#designation_status').val(0);
...
function edit(id){
    $('#popup_title').html('');
    $('#popup_btn').html('');
    $('#popup_title').html('Edit Designation');
    ...
    type: "POST",
    cache : false,
    url: "<?php echo base_url(); ?>index.php/backend/master/designation/edit",
    data: dataString,
    ashync: false,
    dataType:'json',
    success: function(data){ 
        $('#hidden_id').val(data.designation_id); 
        $('#designation').val(data.designation);
        $('#designation_status').val(data.designation_status);
       }
     });
}
</script>